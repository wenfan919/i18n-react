...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            actions.I18n.updateState({ showLoading:true })
            if(param){
        ...
        async delItem(param,getState){
            ...
            let res=processData(await api.delI18n(param.param),'删除成功');
            actions.I18n.loadList();
        },

        async downItem(param,getState){
            actions.I18n.updateState({
              showLoading:true
            })
            let res=processData(await api.downI18n(param.param),'下载成功');
            // actions.I18n.loadList();
        ...
        async operation(param,getState){
            actions.I18n.updateState({
              showLoading:true
            })
            let res=processData(await api.operation(param.param),'执行成功');
            // actions.I18n.loadList();
            // console.log("保存信息",res);
            if(res){
               window.history.go(-1);
        ...
        async save(param,getState){//保存
            ...
            let res = processData(await api.saveI18n(param),'保存成功');
            console.log("保存信息",res);
            if(res){
               window.history.go(-1);
            }
